...
### UI/UX Decisions
The platform features 8 landscape-inspired themes (theme switcher UI hidden for now). Navigation uses a traditional sidebar organized into sections: (top) My Workspace, WORK (Jobs, Tasks, Equipment), PEOPLE (Employees, Customer Hub, Hiring), COMPANY (Messages, SOP Library, Quizzes, CompanyHQ), ADMIN (Admin Panel, Tools, Forms). Admin section is only visible to Admin role. "Customer Hub" sidebar label reads "Resource Library" for staff users. Calendar is accessed via the top bar icon (opens as full-screen overlay). All navigation elements adhere to strict sizing and styling guidelines. Layout rules dictate consistent bottom padding, button placement, sidebar item length, and exclusion zones for floating elements. Consistent hover effects are implemented across all clickable elements. Admin Panel navigation is organized into 4 categories (People, Content, AI & Tools, System) with sub-tabs in each. Dashboard ("My Workspace") is a customizable widget-based homepage — see feature spec below.
...
- **Marketing Campaigns**: Full database-backed campaign management with CRUD operations. Features summary dashboard (total spend, total leads, avg cost/lead, active campaign count computed from real data), bar chart performance visualization, create/edit/delete campaigns with platform, budget, spend, leads, and status tracking. Routes: `/marketing`, API: `/api/campaigns`. Table: `campaigns`.
...
- **Customizable Dashboard**: Widget-based homepage at `/` for internal staff (Admin, Manager, Crew). Users add/remove/reorder/resize widgets from a role-filtered picker. Widget layout saved per-user in `users.dashboard_widgets` (jsonb). Desktop: multi-column responsive grid (4 cols). Mobile: single-column stacked list with hold-and-drag reorder. 15 widget types: Messages, Tasks, Work Pipeline, Estimates, Calendar, Equipment, Marketing, Team, SOPs, Quizzes, Materials, Suggestions, Quick Links, Company HQ, Help. Each widget fetches live data independently via React Query. Default widget sets per role on first visit. Customer role bypassed — redirects to Customer Hub. Files: `client/src/pages/Home.tsx`, `client/src/components/dashboard/widgetRegistry.ts`, `client/src/components/dashboard/widgets.tsx`. API: GET/PUT `/api/dashboard-config`.
- **Settings Page**: Centralized settings at `/settings` with sections: Profile & Account (name/email/phone/password), Notifications (email toggle), Language & Display (EN/ES), Appearance (theme picker), Admin Settings (link to Admin Panel, Admin only). Accessible from profile dropdown. Route: `/settings`. File: `client/src/pages/Settings.tsx`.
- **Activity Log System**: Backend activity tracking via `activity_log` table with `seenBy` JSON array for per-user seen tracking. Events logged for: job stage changes, estimate creation/conversion, messages, calendar events, document uploads, work requests. Helper: `server/activityLogger.ts` (`logActivity(eventType, description, link?, userId?)`). API: GET `/api/activity-log`, GET `/api/activity-log/unseen-count`, POST `/api/activity-log/mark-seen`. Migration: `server/activityLogMigration.ts`.
- **Bell Notification Panel**: Enhanced `UpdatesPopup.tsx` with Activity tab (real data from `activity_log` table with colored icons per event type, seen/unseen tracking, "Mark all seen" button) and Updates tab (admin inline posting via "+ Post Update" button with title/description/category form). Bell badge count = unseen activity items + unseen app updates + unread staff notifications.
