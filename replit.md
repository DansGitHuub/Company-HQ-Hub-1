...
- **Navigation**: Traditional sidebar with labeled sections and "Tile View" with Grid, Radial, and Dock layouts. Sidebar sections: FIELD OPS (Dashboard, Jobs, Equipment, Materials, Tools, To-Do List), PEOPLE (Hiring, Employees), KNOWLEDGE (SOP Library, Quizzes, Forms, Help), CUSTOMERS (Customer Hub), COMPANY (CompanyHQ, Marketing, Integrations), ADMIN (Admin Panel). Plow Mapper accessible from within Tools module (not a standalone sidebar item). Role-based section visibility: Crew/New Hire see FIELD OPS + KNOWLEDGE; Crew Lead adds PEOPLE; Manager/HR/Sales see all except ADMIN; Admin sees all. Care Guides removed from main sidebar (accessible inside Customer Hub only). Active module highlighted in sidebar. All sidebar nav items use fixed height (h-9/36px), text-[13px], leading-[16px], h-4 w-4 icons for uniform sizing.
...
- **User Tools**:
    ...
    - **Document Sharing**: External share links for documents (HQ files, employee docs, hiring docs). Features: token-based public access at `/shared/:token` with Chapin Landscapes branding, optional password protection (enforced on both view and download), configurable expiration (24h/48h/7d/30d/90d), custom notes, view count tracking, IP/user-agent access logging. Admin-only link creation via `ShareExternallyDialog`. Admin Panel "Shared Links" tab (`SharedLinksManager`) for managing all links with status badges, copy/revoke actions, and expandable access logs. HQ files served through public download proxy (no auth redirect). Tables: `shared_links`, `shared_link_access_logs`. Migration in `server/sharedLinksMigration.ts`. Frontend: `client/src/pages/SharedDocument.tsx`, `client/src/components/ShareExternallyDialog.tsx`.
