...
- **User Tools**:
    ...
    - **AI Assistant (Phase 1)**: Floating AI assistant panel powered by OpenAI GPT-4o with function calling. Features: circular sparkle button (bottom-right), 380px side panel on desktop / full-width on mobile, conversation history per session, context-aware suggestion chips on open (overdue tasks, P1 alerts, unacknowledged tasks), navigateTo tool for app navigation, 3-layer system prompt (user identity, behavioral rules, app context), conversation logging to assistant_conversations table. Special message components ready for Phase 2: ActionPreviewCard, ActionResultCard, SearchResultCard, ErrorCard, SuggestionChips. Routes in `server/assistantRoutes.ts`. Migration in `server/assistantMigration.ts`. Frontend in `client/src/components/AIAssistantPanel.tsx` and `client/src/components/AssistantMessageCards.tsx`. Tables: assistant_conversations, assistant_agents.
