...
- **My Day (Crew Mobile View)**: Mobile-first page at `/my-day` (WORK nav, all roles). Shows today's assigned jobs with customer name, address, division color-coded border, scheduled time, and work area chips. Tapping a chip opens a clock-in confirmation dialog. "Drive Time", "Shop Time", and "Break" quick chips on every job card. Active-entry banner with live elapsed timer and Clock Out button. Auto-clocks-out any active entry before clocking into a new one. Time log section shows today's entries with duration. Polls every 30 s. API: `GET /api/my-day`, `GET /api/my-day/time-entries` (server/myDayRoutes.ts). Reuses existing `/api/time/clock-in` and `/api/time/clock-out`.
...
- **Settings Page**: Full admin settings hub at `/settings` (Admin/Manager only, Cog link at sidebar bottom). Four tabs: (1) Work Areas — full CRUD for `work_area_types` with division grouping, active toggle, add/edit/delete modal; (2) Divisions — color-swatch editor for division hex colors stored in `app_settings.division_colors`; (3) Estimate Templates — full CRUD for `estimate_templates` (name, type, message, terms, active); (4) Company Info — company name, phone, email, address, website, default tax rate, payment terms stored in `app_settings.company_info`. Backend: `server/settingsRoutes.ts` (GET/PUT `/api/settings/:key`), `server/migrations/appSettings.ts` (creates `app_settings` table). Enhanced existing routes: work-area-types and estimate-templates support `?all=true` to include inactive records; added DELETE `/api/work-area-types/:id` (blocks if in use) and POST/PUT/DELETE `/api/estimate-templates`.
