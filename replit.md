...
- **Employee Management**: Employee Portal for payroll, time off, and personal details. Includes **Employee File Forms** system: (1) **Time Off Requests** — real DB-connected form in Employee Portal (vacation/sick/personal/unpaid/bereavement), admin/manager Approve/Deny from employee file with email notification; (2) **Resignation Letter** — signature pad form in Employee Portal with in-app + email notification to all admins/managers on submit, viewable from employee file; (3) **Corrective Action Report** — dual-signature form (employee acknowledgment + manager), permission-filtered by role (Manager→Crew only, Admin→anyone), accessible from Documents tab in Employees.tsx; (4) **Reorganized DocumentsTab** — 4 sections: Onboarding Forms, Employment Documents, Corrective Actions (with Issue Report button), Employee-Initiated Forms (time off + resignation). New API routes registered in `server/employeeFormsRoutes.ts`. Tables: `time_off_requests`, `resignation_letters`, `corrective_actions`. Shared SignaturePad component: `client/src/components/forms/SignaturePad.tsx`. Employee Portal gained `/api/employees/me` route. Email functions: `sendTimeOffRequestEmail`, `sendResignationNotificationEmail` in `server/email.ts`.
