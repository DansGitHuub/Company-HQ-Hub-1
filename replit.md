...
- **Interactive Calendar**: Full calendar module with month/week/day/list views, event CRUD with role-based permissions, event types (personal, job, meeting, deadline, maintenance, company), Google Calendar OAuth sync (connect/disconnect per user, auto-push events, pull/sync Google Calendar events into CompanyHQ display with "Sync Now" button), team member assignment, company-wide events, contact fields (name, email, phone) on events, search and filtering with Google Calendar filter option. Google events shown in amber color, read-only in detail view. Routes: `/calendar`, API: `/api/calendar/*`, `/api/calendar/google/sync`, `/api/calendar/google/events`. Tables: `calendar_events` (with `contact_name`, `contact_email`, `contact_phone` columns), `google_calendar_events`.
