...
## Before adding a new FK column, verify the referenced column's real type — don't trust schema.ts

`shared/schema.ts` declarations can be wrong about a column's actual live type (e.g. it may declare `varchar(36)` for a column that Postgres actually stores as `uuid`, because the column was originally created via raw SQL with `gen_random_uuid()` and schema.ts was written/guessed later without checking).

**Why:** A new table's FK column must exactly match the referenced column's real Postgres type or the whole `CREATE TABLE` is rolled back with a `cannot be implemented` FK error. Trusting schema.ts's declared type instead of the live DB caused a wasted migration attempt.

**How to apply:** Before writing a migration that adds a foreign key, run `SELECT column_name, data_type FROM information_schema.columns WHERE table_name = '<referenced_table>'` and match the FK column's type to that real result, not to what `shared/schema.ts` claims.
